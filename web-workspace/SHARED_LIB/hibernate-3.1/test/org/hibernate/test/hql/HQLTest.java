//$Id: HQLTest.java,v 1.173 2005/12/07 22:04:31 steveebersole Exp $
...
public class HQLTest extends QueryTranslatorTestCase {
	...
	public void testExpressionInFunction() throws Exception {
		...
		assertTranslation( "from Animal an where an.bodyWeight > abs(3+5)" );
		assertTranslation( "from Animal an where an.bodyWeight > abs(3*5)" );
		SQLFunction concat = (SQLFunction) getDialect().getFunctions().get("concat");
...
//		assertTranslation( "from Animal an where an.offspring.mother.bodyWeight > 10" );
	...
	public void testExceptions() throws Exception {
		...
		QuerySyntaxException qse = new QuerySyntaxException( new RecognitionException( "test" ), "from bozo b where b.clown = true" );
...
//		 GROUP BY is okay.
	...
	*/
	...
	private void compileWithAstQueryTranslator(String hql, boolean scalar) {
		...
		QueryTranslator newQueryTranslator = ast.createQueryTranslator( hql, Collections.EMPTY_MAP, factory );
	...
	public void testNestedAssociationPropertyNoAlias() throws Exception {
		String msg = null;
		try {
			compileWithAstQueryTranslator("from Animal where zoo.address.street = '123 Bogus St.'", false);
		}
		catch (QueryException e) {
			msg = e.getMessage();
		}
		assertNotNull(msg);
		assertTrue(msg.indexOf("is not a component") >= 0);
	}
